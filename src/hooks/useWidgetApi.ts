...
interface WidgetActions {
  refreshTasks: (filters?: { agent?: string; status?: string }) => void;
  getTaskDetail: (taskId: string) => Promise<Task | null>;
  startTask: (taskId: string, mode: 'review' | 'auto') => Promise<boolean>;
  approveTask: (taskId: string) => Promise<boolean>;
}

export function useWidgetApi(): [WidgetState, WidgetActions] {
  ...
  const pendingRef = useRef<Map<string, { resolve: (data: unknown) => void; timer: ReturnType<typeof setTimeout> }>>(new Map());
  ...
  useEffect(() => {
    ...
    api.on(`action:${EVENT_TASK_DATA}`, (ev: any) => {
    ...
    api.on(`action:${EVENT_TASK_RESPONSE}`, (ev: any) => {
      const content = ev.detail?.data?.content ?? {};
      const requestId = content.request_id;
  ...
  function sendEvent(api: WidgetApi, eventType: string, content: Record<string, unknown>): string {
  ...
  function waitForResponse(requestId: string, timeoutMs = 10000): Promise<unknown> {
  ...
  const refreshTasks = useCallback((filters?: { agent?: string; status?: string }) => {
  ...
  const getTaskDetail = useCallback(async (taskId: string): Promise<Task | null> => {
    ...
    try {
      const resp = await waitForResponse(requestId) as { task?: Task };
  ...
  const startTask = useCallback(async (taskId: string, mode: 'review' | 'auto'): Promise<boolean> => {
    const api = widgetApiRef.current;
    if (!api) return false;
    const requestId = sendEvent(api, EVENT_TASK_START, { task_id: taskId, mode });
    try {
      const resp = await waitForResponse(requestId) as { result?: { ok: string } };
  ...
  const approveTask = useCallback(async (taskId: string): Promise<boolean> => {
    ...
    try {
      const resp = await waitForResponse(requestId) as { ok?: boolean };
